...
export const produceValue = (baseValue: number, specified: string): [number, string] | number | null => {
    const digitMatch = specified.match(/^(-?\d+(?:\.\d+)?)(px|%)$/);
    ...
    const asteriskMatch = specified.match(/^(-?\d+(?:\.\d+)?)?([a-z]+)$/);
...
export const smartProduceValue = (value: string, baseValue: number, customUnits: Units): [number, string] | number | null => {
